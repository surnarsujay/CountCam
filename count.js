...
// Function to insert data into MSSQL database with duplicate check
async function insertIntoDatabase(mac, sn, deviceName, enterCarCount, enterPersonCount, enterBikeCount,
    ...
    try {
        ...
        // Check if an entry with the same 'sn' already exists
        const checkQuery = `
        ...
        if (result.recordset.length > 0) {
            ...
            console.log(`Entry with sn: ${sn} already exists. Skipping insertion.`);
        } else {
            // Proceed with the insertion if no duplicate entry is found
            const insertQuery = `
